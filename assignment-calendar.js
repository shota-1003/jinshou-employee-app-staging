        ...
        function buildEntryForm(box, draft, sites) {
            ...
            function renderMembers() {
                ...
                if (!selected.length && !rest.length) {
                    ...
                    if (q) {
                        // 2026-09-15: 外注(下の枠)は名前を自由入力で追加できるが、自社の社員は
                        // 社員マスターから選ぶだけで、この場で新規作成することはできない仕様。
                        // 未登録の新入社員を探して見つからない状態で終わると「追加できない」と
                        // 誤解される(実例: 光市郎さんが森長拓也さんを探して見つからなかった)ため、
                        // 先に社員名簿へ登録が必要であることをその場で案内する。
                        memTokens.append(el('div', 'ac-schedmeta',
                            '新しく入った社員がここに出ない場合は、まず「社員名簿」の「＋ 新入社員を登録する」で登録してください。登録すると、ここに選択肢として出てきます。'));
                    }
                }
            }
